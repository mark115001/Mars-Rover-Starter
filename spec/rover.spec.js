...
// 10 ------- WORKS WITH "MODE", "MOVE", "STATUS_CHECK"
    it("responds correctly to the status check command", function () {
        ...
        console.log(response.message).toBe('TA Power')
        console.log(response.results[0][0].completed).toBeTruthy
        console.log(response.results[1][0].roverStatus.position).toBe(4321)
        console.log(response.results[2][0].completed).toBeTruthy
        console.log(response.results[3][0].completed).toBeFalsy
        console.log(response.results[4][0].roverStatus.position).toEqual(4321)
        console.log(response.results[4][0].roverStatus.mode).toEqual("LOW_POWER")
        console.log(response.results[4][0].roverStatus.generatorWatts).toEqual(110)
      });

//11 ------- WORKING WITH MODE TO LOW_POWER AND STATUS_CHECK
  it("responds correctly to the mode change command", function () {
    expect(response.results[2][0].completed).toBeTruthy
...
// 12 ------- WORKING WITH MODE CHANGE TO LOW POWER AND A FALSE MOVE
    it("responds with a false completed value when attempting to move in LOW_POWER mode", function () {
      expect(response.results[3][0].completed).toBeFalsy
});
...
// 13 -------   WORKING---STATUS CHECK, LOW POWER, STATUS CHECK
  it("responds with the position for the move command--MOVE command will update the rover's position with the position value in the command", function () {  
      expect(response.results[0][0].completed).toBeTruthy
